...
> Read `<project-root>/.refactor-arch/phase-2-audit.md` (the audit findings), then read `references/architecture-guidelines.md` and `references/refactoring-playbook.md` from the skill. Restructure `<project-root>` into the MVC layout described in the architecture guidelines — adapting folder names to this project's language/framework convention, and respecting the "adapting to a partially-organized project" section if the project already has some layering. For every finding in the audit report, apply the matching playbook transformation (or the closest one, using its intent, if no exact match exists). **Hard gate — not optional:** every mutable route (POST/PUT/PATCH/DELETE) that the audit report marked CRITICAL for missing authentication/authorization MUST receive an auth guard in this phase, regardless of whether its path starts with `/admin/` — this applies to admin and non-admin routes alike. Preserve the original public API surface — same routes, methods, and request/response shapes — this is a structural refactor, not a contract change; but "preserve the surface" means paths/methods/shapes, it does NOT mean preserving the *absence* of an auth check the audit flagged CRITICAL. A route that used to respond 200 with no credentials and now correctly responds 401/403 without them is the intended fix, not a contract break. Do not narrow this scope on your own judgment — if you believe a CRITICAL missing-auth finding should be left unfixed for some reason, stop and ask the human (AskUserQuestion) instead of silently documenting an invented scope decision. Then actually validate the result: install dependencies if the manifest changed, boot the application in the background, hit every original endpoint with a real request (curl or an HTTP client), confirm each responds the same way it did before the refactor or better — e.g. any route the audit flagged CRITICAL for missing auth, admin or not, now correctly requires it — and stop the background process afterward. Write a validation report to `<project-root>/.refactor-arch/phase-3-validation.md` covering: the new directory tree, which findings were fixed, boot output, and the endpoint-by-endpoint check results. Return that report as your final message.
